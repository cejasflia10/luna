...
/* ===== Banco: lectura simple ===== */
function bank_get(){
  global $conexion, $db_ok; if(!$db_ok) return ['holder'=>'','bank_name'=>'','alias'=>'','cbu'=>''];
  // no falla si no existe: la crea y asegura fila
  @$conexion->query("CREATE TABLE IF NOT EXISTS `bank_info` (
...
$has_image_url = $has_products && hascol('products','image_url');
$has_variant_price = $has_variants && hascol('product_variants','price');
$has_product_price = $has_products && hascol('products','price');
...
if (!isset($_SESSION['payment']) || !is_array($_SESSION['payment'])) {
  $_SESSION['payment'] = [
    'method'=>'efectivo','installments'=>1,'delivery_method'=>'retirar',
    'reserve72'=>0,
    'address'=>['fullname'=>'','phone'=>'','street'=>'','number'=>'','city'=>'','notes'=>''],
    'receipt_url'=>''
  ];
}
...
if (($_SERVER['REQUEST_METHOD']??'')==='POST'){
  ...
  if ($action==='add' && $pid>0 && $qty>0){
    ...
    $payment = [
      ...
      'delivery_method'=>($_POST['delivery_method']??'retirar'),
...
foreach($cart as $row){
  ...
  $items[]=['pid'=>$pid,'vid'=>$vid,'name'=>$name,'img'=>$img,'price'=>$price,'qty'=>$qty,'line_total'=>$line_total,'attrs_html'=>($attrs?'<div class="muted">'.implode(' · ',$attrs).'</div>':'')];
}
$cart_count = array_sum(array_column($cart,'qty')); $_SESSION['cart_count']=$cart_count;
...
$fee=0; $discount=0; $total=max(0,$subtotal+$fee-$discount); $cuota_monto=($cuotas>1)?($total/$cuotas):0;

/* ===== Cloudinary (comprobante) ===== */
function envv($k){ if(isset($_ENV[$k])&&$_ENV[$k]!=='')return $_ENV[$k]; if(isset($_SERVER[$k])&&$_SERVER[$k]!=='')return $_SERVER[$k]; $v=getenv($k); return $v!==false?$v:null; }
$CLD_NAME=envv('CLOUDINARY_CLOUD_NAME')?:''; $CLD_PRESET=envv('CLOUDINARY_UPLOAD_PRESET')?:''; $CLD_FOLDER=envv('CLOUDINARY_FOLDER')?:'luna-shop/comprobantes';
...
<head>
  ...
  <style>
    ...
    .grid{display:grid;grid-template-columns:1fr 360px;gap:14px} @media (max-width:900px){.grid{grid-template-columns:1fr}}
    .card{background:var(--card,#12141a);border:1px solid var(--ring,#2d323d);border-radius:12px;overflow:hidden}
    .p{padding:12px}.row{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
    .badge{display:inline-block;padding:.2rem .5rem;border:1px solid var(--ring);border-radius:.5rem;font-size:.8rem;opacity:.9}
    .cta{display:inline-block;padding:.5rem .9rem;border:1px solid var(--ring);border-radius:.6rem;text-decoration:none;background:transparent;color:inherit;cursor:pointer}
    .muted{opacity:.85}.qty{width:90px}.pay-card{background:var(--card,#12141a);border:1px solid var(--ring,#2d323d);border-radius:10px;padding:12px}
    .input{min-width:120px}.field{display:flex;flex-direction:column;gap:4px;flex:1}.two{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .hide{display:none}.ok{color:#22c55e}.err{color:#ef4444}
  </style>
</head>
<body>
        ...
        <form action="<?= urlc('carrito.php') ?>" method="post" id="payForm">
          ...
          <div class="pay-card">
            ...
            <div style="margin-top:10px">
              <label class="badge"><input type="checkbox" name="reserve72" <?= ($reserve72?'checked':'') ?>> Reservar productos por 72 hs</label>
            </div>
          </div>
          ...
          <div class="pay-card">
            ...
            <!-- Débito/Transferencia: mostrar datos -->
            <div id="debitBlock" class="<?= ($method==='debito'?'':'hide') ?>" style="margin-top:10px">
              <div class="pay-card" style="background:rgba(255,255,255,.02)">
                <div><b>Titular:</b> <?= $BANK['holder'] ? h($BANK['holder']) : '<span class="muted">—</span>' ?></div>
                <div style="margin-top:4px"><b>Banco:</b> <?= $BANK['bank_name'] ? h($BANK['bank_name']) : '<span class="muted">—</span>' ?></div>
                <div style="margin-top:4px"><b>Alias:</b> <?= $BANK['alias'] ? '<code>'.h($BANK['alias']).'</code>' : '<span class="muted">—</span>' ?></div>
                <?php if (!empty($BANK['cbu'])): ?><div style="margin-top:4px"><b>CBU:</b> <code><?= h($BANK['cbu']) ?></code></div><?php endif; ?>
                <div class="muted" style="margin-top:6px">Adjuntá el comprobante (foto/captura). Se valida al preparar el pedido.</div>
